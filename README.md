...
**工具层：本地模型 vs LLM zero-shot**（500 条 test.txt 抽样）

|  | 准确率 | p50 延迟 | p95 延迟 | 成本 / 千条 |
|---|---|---|---|---|
| 本地 CNN | 87.2% | 0.37 ms | 1.07 ms | ≈ $0 |
| LLM zero-shot | 待测 | 待测 | 待测 | 待测 |

口径要写清楚，否则这张表站不住：本地模型逐条测延迟，LLM 走批量再摊到每条
（对 LLM 有利，但那是真实用法）；本地模型在这批数据的训练集上训过，LLM 没见过
（对 LLM 不利）。所以结论只能说「在这个领域的数据上本地模型不输」，
**不能**说「小模型优于大模型」。
...
需要 API key 才能跑，没有 key 时明确报错，不用假数据凑一张好看的表。
...
149 个用例，覆盖：归一化层的 train/eval 一致性、eval 下不坍缩、padding_idx、
