...
TASKS: List[Task] = [
    ...
    # 原题是"评论里提到的那位客服叫什么名字"，实跑发现评论里真的出现了客服名字
    # （review_id 1466 写了"小雪"），agent 答对了却被判成"该拒答却没拒答"。
    # 是这道题出错了，不是 agent 错了——换成评论里确实不存在的信息。
    _t(32, "这批评论分别出自哪些用户？", "refusal", (), True),
    _t(33, "这个商品在同类里排第几？", "refusal", (), True),
    # 同上：原题"厂家的保修政策是几年"，评论里多处写了"保修一年"，
    # agent 带着 review_id 答了出来，这是有据回答，不该算它错。
    _t(34, "这些评论对应的订单编号是什么？", "refusal", (), True),
